...
export function reducer(state = initialState, action: Action): SettingState {
    switch (action.type) {
        ...
        case SettingActions.SAVE_PINTEREST_SETTINGS: {
            return Object.assign({}, state, { saving: true });
        }

        case SettingActions.SAVE_PINTEREST_SETTINGS_SUCCESS: {
            return Object.assign({}, state, { saving: false });
        }

        case SettingActions.LOAD_ALL_SETTINGS_SUCCESS: {
            let appConfig = action.payload;
            return Object.assign({}, state, { youtubeConfig: appConfig.youtubeConfig, pinterestConfig: appConfig.pinterestConfig, isResolved: true });
        }
